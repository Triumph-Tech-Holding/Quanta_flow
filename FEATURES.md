...
| Feature | Status | Endpoint / Arquivo | Notas |
|---|---|---|---|
| Artifact `quanta-flow-mobile` | OK | `artifacts/quanta-flow-mobile/` | Expo SDK 54 + Expo Router, servido em `/mobile/` |
| Login JWT + SecureStore | OK | POST /api/auth/login + `expo-secure-store` | Fallback `localStorage` no preview web |
| AuthContext + useAuthRedirect | OK | `contexts/AuthContext.tsx` | Gate de navegação no `_layout.tsx` raiz |
| Workspace switcher (F39) | OK | GET/POST /api/workspaces[/:id/switch] | Header `x-workspace-id` injetado em todas requisições |
| Dashboard mobile | OK | GET /api/crm/dashboard + /api/brain/insights | KPIs + IA Brain insights |
| Inbox mobile + chat | OK | GET /api/conversations + POST /:id/messages | FlatList invertida, KeyboardAvoidingView |
| CRM mobile + detalhes contato | OK | GET /api/crm/contacts[/:id[/messages]] | Lista com busca + timeline |
| Branding Quanta (Green/Navy/Inter) | OK | `constants/colors.ts` + `@expo-google-fonts/inter` | Splash navy com logo |

**Critério de aceite**: usuário faz login com credencial existente, vê seus workspaces, troca entre eles (JWT reemitido) e acessa Inbox/CRM/Dashboard com dados em tempo real do mesmo backend Express. Pronto para build EAS apontando para o domínio Replit do API server via `EXPO_PUBLIC_DOMAIN`.
