...
- **CHECKLIST FLOW STANDARD (Task #24)**: CLAUDE.md e CHANGELOG.md na raiz do projeto. Tabela `project_status_items` com 25 features pré-seeded. 4 novas abas em Admin/Documentação: CLAUDE.md (viewer inline com renderMarkdown), CHANGELOG (viewer inline), Dicionário de Dados (37 tabelas), Painel de Status (matriz editável com inline-edit, add/delete, progress bars). Nova aba "Protocolos" no Lab: Smoke Tests (10 endpoints críticos com "Executar Todos"), Definition of Done (12 critérios com localStorage persist), Erros Comuns (6 padrões com causa+fix). Endpoints: GET /api/documentation/claude-md, GET /api/documentation/changelog, CRUD /api/admin/project-status.
