...
- **Inbox Module**: Unified messaging center supporting WhatsApp integration via Z-API, Baileys, and Meta Oficial (Cloud API). Features include real-time conversation viewing, sending/receiving messages, automatic webhook configuration, and Fila de Atendimento with SLA timer.
...
- **CHECKLIST FLOW STANDARD (Task #24)**: LAB = cockpit técnico interno (engenharia/governança), não área de testes. CLAUDE.md e CHANGELOG.md na raiz. Tabela `project_status_items` com 25 features pré-seeded e auto-migração no startup. 3 novas abas em Admin/Documentação: CLAUDE.md (viewer inline com renderMarkdown), CHANGELOG (viewer inline), Dicionário de Dados (37 tabelas mapeadas do schema Drizzle). 3 novas abas no Admin/Lab: **Progresso** (Painel de Status — matriz editável de features com inline-edit, add/delete, progress bars — primeira aba do LAB pois é o cockpit), **Protocolos** (Smoke Tests com 10 endpoints críticos + "Executar Todos", Definition of Done com 12 critérios e localStorage, Erros Comuns com 6 padrões causa+fix), e **Docs** (visualizador de toda documentação técnica + download PDF). Endpoints: GET /api/documentation/claude-md, GET /api/documentation/changelog, CRUD /api/admin/project-status.
- **Documentação Técnica do LAB (8 documentos)**: 8 arquivos markdown na raiz acessíveis via Lab → Docs com viewer inline e download PDF: CLAUDE.md (diretrizes), CHANGELOG.md (histórico), FEATURES.md (catálogo de features por módulo), STORIES.md (50+ user stories por persona/épico), DICTIONARY.md (dicionário de dados das 37 tabelas), VISUAL_FLOW.md (diagramas ASCII/Mermaid de arquitetura, fluxo de mensagens, pipeline, blocos de automação), TESTING.md (estratégia, matriz E2E, smoke tests, DoD, erros comuns), DEPLOY_GUIDE.md (procedimento de deploy, env vars, migrations, rollback, checklist). Endpoints genéricos: GET /api/documentation/tech (lista), GET /api/documentation/tech/:name (markdown), GET /api/documentation/tech/:name/pdf (PDF via pdfkit). Whitelist server-side previne acesso a arquivos arbitrários.
- **Social/Ads — Estúdio de Conteúdo Omnichannel**: Content creation studio for social media and ads. Features: Project CRUD (scoped by userId with brand colors), AI content generation via gpt-4o-mini (formats: headlines, caption, hooks, socialAds, email, blogPost), TTS audio generation via tts-1 (stored in /uploads/social-audio/), UTM link builder (source/medium/campaign params), filterable content library, grouped calendar view (Record<date, Record<channel, ContentAsset[]>>), publication schedules, dashboard metrics. RBAC: admin/super_admin for all write/generate operations; all authenticated users have read access to library/calendar (scoped by userId). Asset ownership enforced via strict non-null check (userId required on all social assets). Data model uses UUID primary keys. **Chat Wizard MFORTE** (POST /api/admin/social/wizard/start): enriches idea with ideaArea/ideaSources/3 headlines via GPT-4o-mini, Step 1 is conversational chat. **Clonagem de Voz e Avatar (Task #22)**: `cloningIds` stored in project `brand` JSONB (elevenLabsApiKey, elevenLabsVoiceId, heygenApiKey, heygenAvatarId); credentials never exposed in GET responses (replaced with hasElevenLabs/hasHeyGen booleans); ElevenLabs TTS via eleven_multilingual_v2 stores elevenLabsAudioUrl in asset formats; HeyGen video via /v2/video/generate stores heygenVideoId/heygenVideoStatus/heygenVideoUrl in formats; async status polling via /heygen-status endpoint. Endpoints: `/api/admin/social/projects`, `/api/admin/social/assets`, `/api/admin/social/generate`, `/api/admin/social/wizard/start`, `/api/admin/social/calendar`, `/api/admin/social/stats`, `/api/admin/social/assets/:id/tts`, `/api/admin/social/assets/:id/elevenlabs-tts`, `/api/admin/social/assets/:id/heygen-video`, `/api/admin/social/assets/:id/heygen-status`, `/api/admin/social/assets/:id/generate-utm`, `/api/admin/social/assets/:id/schedules`, `/api/admin/social/schedules/:id`.

### System Design Choices
- **Modular Structure**: Clear separation of client, server, and shared codebases.
- **Database Schema**: Dedicated tables for users, leads, API configurations, conversations (with channel field), messages, settings, roles, permissions, audit logs, unified_contacts (with queueStatus/SLA/activeFlowId), agent_assignments, learning_tracks, learning_deliveries, outbound_webhooks, sheet_integrations, email_configs, ai_agents, documentation_versions, campaigns, campaign_deliveries, message_templates, social_projects (UUID PK, userId-scoped, brand colors), content_assets (UUID PK, userId-scoped, formats JSONB, utmLink, audioUrl, AI-generated content), publication_schedules (UUID PK, scheduled social posts), project_status_items (FLOW Standard feature matrix with priority/status/progress enums). The `automation_flows` table includes an `agent_id` column referencing `ai_agents`. Social tables use UUID primary keys (varchar + gen_random_uuid() default).
...
- **WhatsApp Provider Management**: Flexible system to switch between different WhatsApp providers (Z-API, Baileys, Evolution, Meta Oficial). Meta provider uses WhatsApp Business Cloud API with Graph API v19.0, webhook verification (hub challenge), and incoming message processing. Credentials stored encrypted via settings (meta_phone_number_id, meta_access_token, meta_verify_token).
